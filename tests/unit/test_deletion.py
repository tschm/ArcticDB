...
import pytest
import time
from arcticcxx.storage import NoDataFoundException, KeyType
...
def test_delete_snapshot(version_store_factory):
    ...
    df1 = sample_dataframe(10000)
    lmdb_version_store.write(symbol, df1)
    lmdb_version_store.snapshot(snap)

    df2 = sample_dataframe(10000)
...
def test_tombstones_deleted_data_keys_version_delete(lmdb_version_store_tombstone, sym, delete_order):
...
def test_tombstone_of_non_existing_version(lmdb_version_store_tombstone, sym):
    ...
    version_loader = loader.VersionLoader(lib)
    version_loader.write_version(sym, 0, 0)
    version_loader.write_version(sym, 1, 1)
    version_loader.write_version(sym, 3, 3)
    version_loader.write_version(sym, 5, 5)
