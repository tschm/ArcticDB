...
from arcticcxx.version_store import TailRange as _TailRange
...
def test_tail_default_num_rows(lmdb_version_store_tiny_segment, one_col_df):
    ...
    assert np.array_equal(df.tail(num_rows), lmdb_version_store_tiny_segment.tail(symbol).data)
...
def test_tail_with_column_filter(lmdb_version_store_tiny_segment, three_col_df):
    ...
    assert np.array_equal(
        df.filter(items=columns).tail(num_rows),
