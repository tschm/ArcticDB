...
from arcticcxx.version_store import HeadRange as _HeadRange
...
def test_head_default_num_rows(lmdb_version_store_tiny_segment, one_col_df):
    ...
    assert np.array_equal(df.head(num_rows), lmdb_version_store_tiny_segment.head(symbol).data)
...
def test_head_with_column_filter(lmdb_version_store_tiny_segment, three_col_df):
    ...
    assert np.array_equal(
        df.filter(items=columns).head(num_rows),
