...
def test_read_old_snapshot_data(mongo_version_store):
    original_data = [1, 2, 3]
    modified_data = [1, 2, 3, 4]
    mongo_version_store.write("c", original_data)
    mongo_version_store.snapshot("snap_3")
    mongo_version_store.write("c", modified_data)
    mongo_version_store.snapshot("snap_4")
    assert mongo_version_store.read("c", as_of="snap_3").data == original_data
    assert mongo_version_store.read("c").data == modified_data


def test_snapshot_metadata(mongo_version_store):
    original_data = [1, 2, 3]
    metadata = {"metadata": "Because why not?"}
    snap_name = "meta_snap"
    mongo_version_store.write(snap_name, original_data)
    mongo_version_store.snapshot(snap_name, metadata=metadata)

    assert mongo_version_store.read(snap_name).data == original_data
    all_snaps = mongo_version_store.list_snapshots()
    metadata_for_snap = [meta for snap, meta in all_snaps.items() if snap == snap_name][0]
    assert metadata_for_snap == metadata


def test_snapshots_skip_symbol(mongo_version_store):
    original_data = [1, 2, 3]
    mongo_version_store.write("f", original_data)
    mongo_version_store.write("g", original_data)
    mongo_version_store.snapshot("snap_5", metadata=None, skip_symbols=["f"])
    assert mongo_version_store.read("g", as_of="snap_5").data == original_data
    with pytest.raises(Exception):
        mongo_version_store.read("f", as_of="snap_5")
...
def test_snapshot_explicit_versions(lmdb_version_store):
    ...
    print(lib.list_versions(snapshot="snap_8"))
    print("versions:", lib.list_versions())

    assert lib.read("i", as_of="snap_8").data == original_data
    assert lib.read("j", as_of="snap_8").data == modified_data


def test_list_symbols_with_snaps(mongo_version_store):
    original_data = [1, 2, 3]

    mongo_version_store.write("s1", original_data)
    mongo_version_store.write("s2", original_data)

    mongo_version_store.snapshot("snap_9")
    mongo_version_store.write("s3", original_data)

    assert "s3" not in mongo_version_store.list_symbols(snapshot="snap_9")
    assert "s3" in mongo_version_store.list_symbols()


def test_list_versions(mongo_version_store):
    original_data = [1, 2, 3]

    mongo_version_store.write("t1", original_data)
    mongo_version_store.write("t2", original_data)

    mongo_version_store.snapshot("snap_versions")
    mongo_version_store.write("t1", original_data)

    all_versions = mongo_version_store.list_versions()
