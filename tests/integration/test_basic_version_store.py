...
def test_date_range_nanoseconds(lmdb_version_store):
    sym = "date_test_3"

    # Dataframe has three rows, indexed by timestamps 1 nanosecond apart
    ...
    index = pd.DatetimeIndex([timestamp_0, timestamp_1, timestamp_2])
    df = pd.DataFrame(data=[0, 1, 2], index=index)

    lmdb_version_store.write(sym, df)

    # Try to read all rows
    all_rows_result = lmdb_version_store.read(sym, date_range=DateRange(timestamp_0, timestamp_2)).data
    assert_frame_equal(all_rows_result, df)

    # Try to read only the first row
    first_row_result = lmdb_version_store.read(sym, date_range=DateRange(timestamp_0, timestamp_0)).data
    assert_frame_equal(first_row_result, df.iloc[[0]])

    # Try to read first and second rows
    first_and_second_row_result = lmdb_version_store.read(sym, date_range=DateRange(timestamp_0, timestamp_1)).data
    assert_frame_equal(first_and_second_row_result, df.iloc[[0, 1]])

    # Try to read second and third rows
    second_and_third_row_result = lmdb_version_store.read(sym, date_range=DateRange(timestamp_1, timestamp_2)).data
    assert_frame_equal(second_and_third_row_result, df.iloc[[1, 2]])
...
def test_update_date_range_ahl_timeseries(lmdb_version_store_ts_norm):
    ...
    ts = timeseries.TimeSeries(datetimes, a)
    lmdb_version_store_ts_norm.write("sym_1", ts)

    datetimes = [datetime(2022, 5, 1) + timedelta(days=i) for i in range(41)]
    a = np.arange(0, len(datetimes))
    update_ts = timeseries.TimeSeries(datetimes, a)

    # when
    lmdb_version_store_ts_norm.update("sym_1", update_ts, date_range=(datetime(2022, 6, 2), datetime(2022, 6, 4)))

    # then
    result = lmdb_version_store_ts_norm.read("sym_1").data
    np.testing.assert_array_equal(result.values, [1, 32, 33, 34, 5])
...
@pytest.mark.parametrize("factory_name", ["version_store_factory", "mongo_store_factory"])
...
def test_library_deletion_mongo(mongo_version_store):
    # lmdb uses fast deletion
    mongo_version_store.write("a", 1)
    mongo_version_store.write("b", 1)

    mongo_version_store.snapshot("snap")
...
def test_batch_read_meta_multiple_versions(mongo_version_store):
...
@pytest.mark.skip
def test_merge_read_df(lmdb_version_store_new):
    lib = lmdb_version_store_new

    initial_timestamp = pd.Timestamp("2019-01-01")
    second_ts = initial_timestamp + pd.DateOffset(10)
    final_ts = second_ts + pd.DateOffset(10)

    df1 = pd.DataFrame(data=np.arange(0, 5), index=pd.date_range(initial_timestamp, periods=5))
    df2 = pd.DataFrame(data=np.arange(5, 15), index=pd.date_range(second_ts, periods=10))
    df3 = pd.DataFrame(data=np.arange(15, 25), index=pd.date_range(final_ts, periods=10))
    lib.write("sym1", df1)
    lib.write("sym2", df2)
    lib.write("sym3", df3)
    merged_df = lib._merge_read(["sym1", "sym2", "sym3"]).data

    assert len(merged_df) == len(df1) + len(df2) + len(df3)
    assert merged_df.index[0] == initial_timestamp
    assert merged_df.index[-1] == final_ts + pd.DateOffset(9)
    assert_array_equal(merged_df[0], np.arange(0, 25))


@pytest.mark.skip
def test_merge_read_df_interleaved(lmdb_version_store_new):
    lib = lmdb_version_store_new

    first_ts = [pd.Timestamp("2019-01-01"), pd.Timestamp("2019-01-03"), pd.Timestamp("2019-01-05")]
    second_ts = [pd.Timestamp("2019-01-02"), pd.Timestamp("2019-01-04"), pd.Timestamp("2019-01-06")]

    df1 = pd.DataFrame(data=np.array([1, 3, 5]), index=first_ts)
    df2 = pd.DataFrame(data=np.array([2, 4, 6]), index=second_ts)
    lib.write("sym1", df1)
    lib.write("sym2", df2)
    merged_df = lib._merge_read(["sym1", "sym2"]).data

    assert len(merged_df) == 6
    assert merged_df.index[0] == first_ts[0]
    assert merged_df.index[1] == second_ts[0]
    assert_array_equal(merged_df[0], np.arange(1, 7))


@pytest.mark.skip
def test_write_read_partitioned_df(lmdb_version_store_new):
    lib = lmdb_version_store_new

    first_ts = [pd.Timestamp("2019-01-01"), pd.Timestamp("2019-01-03"), pd.Timestamp("2019-01-05")]
    second_ts = [pd.Timestamp("2019-01-02"), pd.Timestamp("2019-01-04"), pd.Timestamp("2019-01-06")]
    third_ts = [pd.Timestamp("2019-01-07"), pd.Timestamp("2019-01-08"), pd.Timestamp("2019-01-09")]

    df1 = pd.DataFrame({"col1": np.array([1, 3, 5]), "partition_col": ["a", "a", "a"]}, index=first_ts)
    df2 = pd.DataFrame({"col1": np.array([2, 4, 6]), "partition_col": ["b", "b", "b"]}, index=second_ts)
    df3 = pd.DataFrame({"col1": np.array([7, 8, 9]), "partition_col": ["c", "c", "c"]}, index=third_ts)
...
def test_compact_version_map_basic(lmdb_version_store_with_write_option, sym):
    ...
    assert len(lib.list_versions(sym)) == 10
    assert len(lt.find_keys_for_id(KeyType.VERSION, sym)) == 2

    for idx in range(10):
        assert lib.read(sym, idx).data == idx
...
def test_compact_version_map_with_delete(s3_version_store_tombstones, sym):
    ...
    assert len(lib.list_versions(sym)) == 10
    assert len(lt.find_keys_for_id(KeyType.VERSION, sym)) == 10

    for idx in random.sample(list(range(10)), 5):
        lib.delete_version(sym, idx)

    lib.version_store._compact_version_map(sym)
    assert len(lib.list_versions(sym)) == 5
    assert len(lt.find_keys_for_id(KeyType.VERSION, sym)) == 2
...
def test_compact_version_map_mixed(mongo_version_store_tombstones, sym):
    ...
    for idx in range(10, 20):
        if idx % 3 == 0:
            lib.delete_version(sym, idx - 10)
        else:
            lib.write(sym, idx)

    lib.version_store._compact_version_map(sym)
    assert len(lib.list_versions(sym)) == 14
    assert len(lt.find_keys_for_id(KeyType.VERSION, sym)) == 2
...
def test_compact_library_mixed(lib_type, sym, request):
    ...
    for idx in range(10):
        lib.write(sym, idx)
    for idx in range(10):
        lib.write(sym + str(idx), idx)
    lib.version_store.compact_library(20)
    assert len(lib.list_versions(sym)) == 10
    assert len(lt.find_keys_for_id(KeyType.VERSION, sym)) == 2
    ...
    for idx in range(10):
        # compact it multiple times using a small batch size
        lib.version_store.compact_library(1)

    # cannot guarantee it will be compacted but number of versions
    # should be consisted
...
def test_batch_restore_version(lmdb_version_store_tombstone_and_sync_passive):
    lmdb_version_store = lmdb_version_store_tombstone_and_sync_passive
    symbols = []
    for i in range(5):
...
def test_dynamic_schema_bucketize(get_wide_df, sym, version_store_factory):
    symbol = sym
    ...
    default_num_bucket = 150

    lib.write(sym, df1)
    assert (
        len(lt.find_keys_for_id(KeyType.TABLE_DATA, sym)) == default_num_bucket
    )  # the default value of number of buckets

    df2 = get_wide_df(1, 40, 40)
    lib.append(sym, df2)
    assert (
        len(lt.find_keys_for_id(KeyType.TABLE_DATA, sym)) == default_num_bucket + 4
    )  # 40 columns => 40/10 more data keys added

    df3 = get_wide_df(2, 400, 400)
    for i in range(3, 50):
        df4 = get_wide_df(i, 400, 400)
        df3 = pd.concat([df3, df4])

    lib.append(sym, df3)
    assert (
        len(lt.find_keys_for_id(KeyType.TABLE_DATA, sym)) == default_num_bucket + 4 + 5 * 40
    )  # 5 segments along rows * 40 segments along cols
    ...
    df_update = get_wide_df(10, 300, 300)
    df5 = pd.concat([df3.head(8), df_update, df3.tail(39)])
    lib.update(sym, df_update)

    res = lib.read(sym).data
    finaldf = pd.concat([df1, df2, df5])
    finaldf = finaldf.reindex(sorted(list(finaldf.columns)), axis=1)
    res = res.reindex(sorted(list(res.columns)), axis=1)
    assert_frame_equal(res, finaldf)
