...
TEST(VersionStore, TestReadTimestampAt) {
  ...
  auto key = get_version_key_from_time(mock_store, version_map, id, timestamp(0), pipelines::VersionQuery{}, ReadOptions{});
  ASSERT_EQ(key.value().content_hash(), 3);

  version_map->write_version(mock_store, key2);
  key = get_version_key_from_time(mock_store, version_map, id, timestamp(0), pipelines::VersionQuery{}, ReadOptions{});
  ASSERT_EQ(key.value().content_hash(), 3);
  key = get_version_key_from_time(mock_store, version_map, id, timestamp(1), pipelines::VersionQuery{}, ReadOptions{});
  ...
  key = get_version_key_from_time(mock_store, version_map, id, timestamp(1), pipelines::VersionQuery{}, ReadOptions{});
  ASSERT_EQ(key.value().content_hash(), 4);
  key = get_version_key_from_time(mock_store, version_map, id, timestamp(2), pipelines::VersionQuery{}, ReadOptions{});
...
TEST(VersionStore, TestReadTimestampAtInequality) {
  ...
  auto version_map = version_store._test_get_version_map();
  version_map->write_version(mock_store, key1);
  auto key = get_version_key_from_time(mock_store, version_map, id, timestamp(1), pipelines::VersionQuery{}, ReadOptions{});
