...
inline void check_normalization_index_match(NormalizationOperation operation,
    ...
    if (operation == UPDATE) {
        util::check_rte(old_idx_kind == IndexDescriptor::Type::TIMESTAMP && new_is_timeseries,
                        "Update will not work as expected with a non-timeseries index");
    } else {
        // TODO: AN-722
        if (new_is_timeseries) {
            if (old_idx_kind != IndexDescriptor::Type::TIMESTAMP) {
            ...
            if (old_idx_kind != IndexDescriptor::Type::ROWCOUNT) {
