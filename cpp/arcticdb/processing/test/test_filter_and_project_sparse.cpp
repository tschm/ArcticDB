...
//    auto& projected_column = segment.column(*column_index);
//
//    // sparse_floats_1 has fewer values than dense_floats_1
//    ASSERT_EQ(rhs_input_column->last_row(), projected_column.last_row());
//    ASSERT_EQ(lhs_input_column->row_count(), projected_column.row_count());
//    ASSERT_TRUE(lhs_input_column->opt_sparse_map() == projected_column.opt_sparse_map());
//
//    for (auto idx=0; idx< rhs_input_column->last_row(); idx++) {
//        if (lhs_input_column->has_value_at(idx)) {
//            ASSERT_TRUE(projected_column.has_value_at(idx));
//            ASSERT_FLOAT_EQ(*lhs_input_column->scalar_at<double>(idx) * *rhs_input_column->scalar_at<double>(idx),
...
//    auto component_manager = std::make_shared<ComponentManager>();
//
//    const std::string lhs_column_name{"sparse_floats_2"};
//    const std::string rhs_column_name{"dense_floats_1"};
//    const std::string output_column{"MUL"};
//
//    auto expression_node = std::make_shared<ExpressionNode>(ColumnName(lhs_column_name), ColumnName(rhs_column_name), OperationType::MUL);
...
//    auto& projected_column = segment.column(*column_index);
//
//    // dense_floats_1 has fewer values than sparse_floats_2
//    ASSERT_EQ(lhs_input_column->last_row(), projected_column.last_row());
//    ASSERT_EQ(lhs_input_column->opt_sparse_map()->rank(7, {}), projected_column.row_count());
//
//    for (auto idx=0; idx< lhs_input_column->last_row(); idx++) {
//        if (lhs_input_column->has_value_at(idx) && idx < 7) {
//            ASSERT_TRUE(projected_column.has_value_at(idx));
//            ASSERT_FLOAT_EQ(*lhs_input_column->scalar_at<double>(idx) * *rhs_input_column->scalar_at<double>(idx),
//                            *projected_column.scalar_at<double>(idx));
//        } else {
