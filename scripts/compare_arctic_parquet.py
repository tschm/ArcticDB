...
def write_to_parquet(df, out_path, compression):
    # arrow_table = pa.Table.from_pandas(df)
    if compression == "UNCOMPRESSED":
        compression = None

    start = time.time()
    df.to_parquet(out_path, engine="fastparquet", compression="lz4")
    elapsed = time.time() - start
    print("parquet write time: " + out_path + " " + str(elapsed))
...
for case, params in cases.items():
    ...
    write_to_parquet(df, path, compression)
